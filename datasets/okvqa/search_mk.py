...
def search_img(img_index, questions_list, seg_list=None, batch_size=16):
    def process_batch(image_paths):
        ...
        distances, indices = search_faiss_index(img_index, query_vectors, k=20)
        return distances, indices
    ...
    for question in questions_list:
        if question['image_id'] in seg_list:
            image_paths_list.append(f"./image_seg/COCO_val2014_{question['image_id']:012d}.jpg")
        else:
            image_paths_list.append(f"./val2014/COCO_val2014_{question['image_id']:012d}.jpg")
...
def search_img_single(img):
    faiss_index_file = '../OVEN/wiki_img.vec'
    csv_file = '../OVEN/wiki_img_index.csv'
...
if __name__ == "__main__":
    ...
    questions_list = []
    file_path = './qa_data/OpenEnded_mscoco_val2014_questions.json'
    # img_set = set()
    ...
    entity_file = './mk/img_mkid_mapping3.json'
    with open(entity_file, 'r') as file:
        entity_dict = json.load(file)
    entity_list = []
    for entity, name in entity_dict.items():
    ...
    if not os.path.exists(seg_result):
        for question in questions_list:
            ...
            image = f"./val2014/COCO_val2014_{question['image_id']:012d}.jpg"
    ...
    if not os.path.exists(img_results_file):
        ...
        for i in range(len(all_indices)):
            question_id = questions_list[i]['question_id']
            ...
            for rank, (dist, idx) in enumerate(zip(distances, indices)):
                if dist < 150:
                    uid = metadata[idx][0]
                    if uid in entity_list and cnt < 3:
    ...
    if not os.path.exists(mk_result_file):
        mk = MetaKnowledge('./mk/mk_data3.json', './mk/mkid_mapping3.json', './mk/img_mkid_mapping3.json')
    ...
    if not os.path.exists(idx_result_file):
        ...
        for i, mk_result in enumerate(mk_results):
            ...
            try:
                ...
                sorted_data = []
                idx_results.append(sorted_data)

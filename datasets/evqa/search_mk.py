...
def search_img(img_index, questions_list, seg_list=None, batch_size=16):
    def process_batch(image_paths):
        ...
        distances, indices = search_faiss_index(img_index, query_vectors, k=4)
        return distances, indices
...
def search_text2(query_input, mk_name, temp_index):
    ...
    distances, indices = temp_index.search(query_vector, k=3)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(img_results_file):
        ...
        index = load_faiss_index(faiss_index_file)
        metadata = load_metadata(csv_file)
        all_distances, all_indices = search_img(index, questions_list)
            ...
            for j, mk_sub_result in enumerate(mk_result):
                ...
                for dist, idx in zip(dists, idxs):
                    for sub_dist, sub_idx in zip(dist, idx):
                        idx_result.append({'question_id': str(question_id), 'name': str(mk_name), 'image': str(img_results[i][j]), 'dist': round(sub_dist.astype(float), 2), 'idx': idx_record[sub_idx.astype(int)]})
            sorted_data = sorted(idx_result, key=lambda x: x['dist'], reverse=False)
            sorted_data2 = []
            temp_idx = -1
            for data in sorted_data:
                if data['idx'] != temp_idx:
                    sorted_data2.append(data)
                    temp_idx = data['idx']
                if len(sorted_data2) >= 3:
                    break

            idx_results.append(sorted_data2)
